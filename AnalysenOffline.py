...
df = spark.read.format("csv").option("inferSchema", "true").option("header","true").option("encoding", "ISO-8859-1").load("/mnt/landi/offline/Offline_O-R.csv")
display(df)

# COMMAND ----------

display(df.select("Lieferdatum", "VerkaufspreisInkl", "Bezeichnung", "Mandant"))
...
dfAF = spark.read.format("csv").option("inferSchema", "true").option("header","true").option("encoding", "ISO-8859-1").option("charset", "ISO-8859-1").load("/mnt/landi/offline/Offline_A-F.csv")
dfGL = spark.read.format("csv").option("inferSchema", "true").option("header","true").option("encoding", "ISO-8859-1").option("charset", "ISO-8859-1").load("/mnt/landi/offline/Offline_G-L.csv")
dfMN = spark.read.format("csv").option("inferSchema", "true").option("header","true").option("encoding", "ISO-8859-1").option("charset", "ISO-8859-1").load("/mnt/landi/offline/Offline_M-N.csv")
dfOR = spark.read.format("csv").option("inferSchema", "true").option("header","true").option("encoding", "ISO-8859-1").option("charset", "ISO-8859-1").load("/mnt/landi/offline/Offline_O-R.csv")
dfSZ = spark.read.format("csv").option("inferSchema", "true").option("header","true").option("encoding", "ISO-8859-1").option("charset", "ISO-8859-1").load("/mnt/landi/offline/Offline_S-Z.csv")
...
appended.write.mode("overwrite").saveAsTable("offlinetrx")

# COMMAND ----------

#creating tables for offline data
dfAF.write.mode("overwrite").saveAsTable("OfflineAF")
dfGL.write.mode("overwrite").saveAsTable("OfflineGL")
dfMN.write.mode("overwrite").saveAsTable("OfflineMN")
dfOR.write.mode("overwrite").saveAsTable("OfflineOR")
dfSZ.write.mode("overwrite").saveAsTable("OfflineSZ")
...
dfoffline.write.mode("overwrite").saveAsTable("offlineTrxCleaned")
...
# MAGIC 
# MAGIC SELECT 
# MAGIC   to_date(o.Lieferdatum, 'dd.MM.yyyy') as Lieferdatum, 
# MAGIC   o.Produkt, 
# MAGIC   to_date(t.Datum, 'dd.MM.yyyy') as Datum,
# MAGIC   o.Liefermenge * o.VerkaufspreisInkl as OfflineUmsatz,
# MAGIC   t.OrderQuantity * t.UnitPrice as OnlineUmsatz
# MAGIC FROM 
...
display(offlineGroupedByProduct)
...
display(offlineGroupedByProductSalesOnDate)
